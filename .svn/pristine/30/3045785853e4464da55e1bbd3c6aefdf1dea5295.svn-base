...
{literal}
function adjust(clientId,dateFrom,dateTo){
  window.open("?controller=CaiWu_Ar_Report&action=adjust&clientId="+ clientId +"&dateFrom="+ dateFrom +"&dateTo="+ dateTo);
}
{/literal}
</script>
...
{webcontrol type='Tmisoptions' model='CaiWu_ArType' selected=$arTypeId}
</select>
业务员：
<select name="traderId" id="traderId">
{webcontrol type='TmisOptions' model='JiChu_Employ' condition='depId=11'}
</select>
