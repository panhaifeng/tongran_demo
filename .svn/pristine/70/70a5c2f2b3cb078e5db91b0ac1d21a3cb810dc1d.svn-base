...
class Controller_JiChu_StCar extends Tmis_Controller {
	var $_modelExample;
	var $title = "松筒车档案";
	var $funcId = 26;
	function Controller_JiChu_StCar() {
		//if(!$this->authCheck()) die("禁止访问!");
		$this->_modelExample = & FLEA::getSingleton('Model_JiChu_StCar');
	}
	...
	function actionRight() {
		...
		$pager =& new TMIS_Pager($this->_modelExample,$condition,'carCode desc');
	...
	function _edit($Arr) {
		...
		$smarty->display('JiChu/StCarEdit.tpl');
	}
