        ...
        <div class="row" style="margin-top:5px;">
            ...
            <div class="col-xs-3">
                <div class="form-control bottom-5">
                    <b>{$item.gongxuName}</b>
                    <input type="checkbox" name='docheck[]' cnt='{$item.gongxuName}' value='{$item.id}' danjia='{$item.danjia}' style="width:15px;height:15px;" {if $item.checked}checked{/if} {if $item.disabled}disabled{/if} />
                </div>
            </div>
            {/foreach}
        </div>
