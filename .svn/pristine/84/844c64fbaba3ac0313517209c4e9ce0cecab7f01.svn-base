...
*  Remark :基于bootstrap的grid插件
*  columns的配置参数如下：
*   $_column = array(
      'compName'=>array('text'=>'送货小车','width'=>200,'checkbox'=>true,'align'=>'right','filed'=>'compName'),
      'compName1'=>array('text'=>'送货小车','width'=>200,'checkbox'=>true,'align'=>'right','filed'=>'compName1'),
      'compName2'=>array('text'=>'送货小车','width'=>200,'checkbox'=>true,'align'=>'right','filed'=>'compName2'),
      'textColumn'=>'标题'
    );
\*********************************************************************/
!function ($) {
    ...
    var BootstrapTable = function (el, options) {
        //2014-9-3 by jeff,保证接口的统一
        //onClickRow的参数不一样
        //onCheck没有
        //onCheckAll没有
        //beforeLoad没有
        if(typeof(options.beforeLoad)=='function') options.queryParams = options.beforeLoad;
        //afterLoad没有
        if(typeof(options.afterLoad)=='function') options.onLoadSuccess = options.afterLoad;
    ...
    //如果data与url同时存在，则优先取sidePagination
    //如果sidePagination未自定义，则默认取data
    BootstrapTable.prototype.initSide = function () {
        var isTSide = false;
        var isUrl=false;
        ...
        if(isUrl && this.options.data){
            isTSide = true;//表示同时设置了url与静态数据
        }

        if(isTSide==false){
            if(this.options.data)this.options.sidePagination = 'client';
            else this.options.sidePagination = 'server';
        ...
        if(this.options.sidePagination == 'server'){
            this.options.data=[];
        }else if(this.options.sidePagination == 'client'){
    ...
    BootstrapTable.prototype.initTable = function () {
        ...
        //给列信息添加数据绑定字段，该列对应的数据显示字段信息
        for(var k in this.options.columns){
            var t = this.options.columns[k];
            if(typeof(t)=='object') {
                ...
                columns.push({'field':k,'text':t});
            }
        }

        //extend之后出现了remove数据，这里暂时取消extend 函数使用，如果出现问题，请纠正2014-10-17 by li
        //暂时未发现其他问题
        this.options.columns = columns;//$.extend({}, columns, {});
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.search) {
            html = [];
            html.push(
                '<div class="pull-right search input-group col-xs-3">',
                    sprintf('<input class="form-control" style="min-width:90px;" type="text" placeholder="%s">',
    ...
    BootstrapTable.prototype.onSearch = function (event) {
        var that = this,
            text = $.trim($(event.currentTarget).parents('.search').find('input').val());
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        var pageNumber = [
            '<span class="btn-group dropup" title="每页显示行数">',
            '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">',
        ...
        html.push(
                ...
                '</ul>',
            '</div>');
        html.push('<div class="pull-right pageRight">'+this.options.pageRight+'</div>');
        html.push('<div class="pull-left"><h5><b>&nbsp;<span class="color-red">'+this.totalPages+'</span>页&nbsp;<span class="color-red">'+this.options.totalRows+'</span>个结果</b></h5></div>');
    ...
    BootstrapTable.prototype.updatePageRight = function(text) {
        if(text) {
            this.options.pageRight = text;
        }
        //修改分页栏右边的内容，2014-9-26 by jeff
        var $right = this.$pagination.find('.pageRight');
        $right.html(this.options.pageRight);
    }
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    //如果是合计行,不显示checkbox,2014-9-29 by jeff
                    // console.log(item);
                    if(item._isHeji) text = '<td class="bs-checkbox"><input type="checkbox" disabled /></td>';
                    else {
        ...
        this.$body.find('tr').off('click').on('click', function () {
            //2014-9-3 update by jeff,参数调整为 rowIndex,rowData
            // debugger;
            // that.trigger('click-row', that.data[$(this).data('index')], $(this));
            that.trigger('click-row', $(this).data('index'),that.data[$(this).data('index')],$(this));
        ...
        this.$selectItem.off('click').on('click', function (event) {
            ...
            //2014-9-3 update by jeff,修改接口参数
            // that.trigger(checked ? 'check' : 'uncheck', row);
            that.trigger(checked ? 'check' : 'uncheck', $(this).data('index'),row);
    ...
    BootstrapTable.prototype.initServer = function () {
        ...
        //判断是否需要分页,2014-9-4 by li
        //默认是使用pageNumber,改为使用pageIndex
        if(this.options.pagination){
            var _params = {
                // pageSize: params.pageSize,
                // pageIndex: params.pageSize * (params.pageNumber - 1),
    ...
    BootstrapTable.prototype.trigger = function (name) {
        ...
        // console.log(name);
        // console.log(this.options);
        // console.log(BootstrapTable.EVENTS[name]);
    ...
    BootstrapTable.prototype.getRowset = function () {
        return this.data;
    };
    ...
    BootstrapTable.prototype.getSelectedRowset = function () {
    ...
    //2014-10-23 by jeff,按索引删除行
    //注意row为数据对象，而不是index
    BootstrapTable.prototype.deleteRow = function (row) {
        // this.data = this.data.concat(data);
        // debugger;
        var rowIndex = $(this.data).index(row);
        if(rowIndex>-1) {
    ...
    $.fn.btGrid = function (option, _relatedTarget) {
        var allowedMethods = [
                'getSelections',
                'load',
                'append',
    ...
    $.fn.btGrid.Constructor = BootstrapTable;
    $.fn.btGrid.defaults = BootstrapTable.DEFAULTS;
    $.fn.btGrid.columnDefaults = BootstrapTable.COLUMN_DEFAULTS;
