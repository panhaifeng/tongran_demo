...
function renderWare() {
	new TmisPopup({
			...
			onSelect: function(json,pos){
				...
				// document.getElementsByName('chufang2WareId[]')[pos].value = json.id;
			}//选择某个记录后的触发动作
		});
}
//选择处方单加选择按虐
function rendeChufang(){
	if ($('#clientId').val()) {
		var url = "?controller=Gongyi_Dye_Chufang&action=PopupChufang"+"&clientId="+$('#clientId').val();
	}
	new TmisPopup({
		obj:document.getElementById('checkChufang'),//进行渲染的目标元素,可以是document.getElementsByName('')得到的数组
		fieldInText:'wareName',//选择后对text控件进行赋值的字段
		fieldInHidden:'id',//选择后对hidden控件进行赋值的字段,默认是id
		width : 120,//渲染后的宽度
		urlPop:url,//弹出框的地址
		titlePop:'选择需要的处方数据',//弹出框的标题
		widthPop:900,
		heightPop:500,
		// idHidden:'guigeId',//创建的hidden元素的id和name
		idBtn:'_btnWare2',//创建的按钮的id
		isArray:true,//if true,创建的元素以[]结尾
		onSelect: function(json){
			if (json.id) {
				SetChufangMx(json.id);
			}

		}//选择某个记录后的触发动作
	});
}
function SetChufangMx(id){
	var url='?controller=Gongyi_Dye_Chufang&action=AjaxGetChufangMx';
	var param={id:id};
	$.getJSON(url,param,function(json){
		if(json.success==false)return false;
		//先把方案和别名显示出来
		$('#selRscfName').val(json['fangan'].gongyiNameRscf);
		$('#selRscf').val(json['fangan'].rscfId);
		$('#memoCf').val(json['fangan'].memoCf);

		$('#selQclName').val(json['fangan'].gongyiNameQcl);
		$('#selQcl').val(json['fangan'].qclId);
		$('#memoQcl').val(json['fangan'].memoQcl);

		$('#selHclName').val(json['fangan'].gongyiNameHcl);
		$('#selHcl').val(json['fangan'].hclId);
		$('#memoHcl').val(json['fangan'].memoHcl);

		$('#selRanseName').val(json['fangan'].gongyiNameRsgy);
		$('#selRanse').val(json['fangan'].rsgyId);
		$('#memoRs').val(json['fangan'].memoRs);
		//执行前处理数据
		if(json['arrQ']!=null){
			json[1]=json['arrQ'];
			var a=json[1].length;
			...
			var i=0;
			 $('#tbl3 input[name="mnemocode[]"]').each(function(){
				if(json['arrQ'][i]==null){return false;}
				if(!json['arrQ'][i].wareId){
					this.value='';
				}
				else{
					this.value=json['arrQ'][i].mnemocode+':'+json['arrQ'][i].wareName+' '+json['arrQ'][i].guige+'        /'+json['arrQ'][i].wareId;
				}
				//下拉框选项处理
				for(var j=0; j<un[i].length;j++){
					var op=un[i].options[j];
					if(op.value==json['arrQ'][i]['unit']){
						op.selected=true;
					}
				}
				unitKg[i].value=json['arrQ'][i].unitKg;
				tmp[i].value=json['arrQ'][i].tmp;
				timeRs[i].value=json['arrQ'][i].timeRs;
				memo[i].value=json['arrQ'][i].memo;
				i++;
			});
		}
		if(json['arrR']!=null){
			...
			renderWare();
			var unitKg=$('#tbl input[name="unitKg[]"]');
			var un=$('#tbl select[name="unit[]"]');
			var tmp=$('#tbl input[name="tmp[]"]');
			var timeRs=$('#tbl input[name="timeRs[]"]');
			var memo=$('#tbl input[name="memo[]"]');
			var j=0;
			...
			var i=0;
			$('#tbl input[name="mnemocode[]"]').each(function(){
				if(json['arrR'][i]==null){return false;}
				if(!json['arrR'][i].wareId){
					this.value='';
				}
				else{
					this.value=json['arrR'][i].mnemocode+':'+json['arrR'][i].wareName+' '+json['arrR'][i].guige+'        /'+json['arrR'][i].wareId;
				}
				//下拉框选项处理
				for(var j=0; j<un[i].length;j++){
					var op=un[i].options[j];
					if(op.value==json['arrR'][i]['unit']){
						op.selected=true;
					}
				}
				unitKg[i].value=json['arrR'][i].unitKg;
				tmp[i].value=json['arrR'][i].tmp;
				timeRs[i].value=json['arrR'][i].timeRs;
				memo[i].value=json['arrR'][i].memo;
				i++;
			});
		}
		if(json['arrH']!=null){
			...
			var i=0;
			$('#tbl2 input[name="mnemocode[]"]').each(function(){
				if(json['arrH'][i]==null){return false;}
				if(!json['arrH'][i].wareId){
					this.value='';
				}
				else{
					this.value=json['arrH'][i].mnemocode+':'+json['arrH'][i].wareName+' '+json['arrH'][i].guige+'        /'+json['arrH'][i].wareId;
				}
				//下拉框选项处理
				for(var j=0; j<un[i].length;j++){
					var op=un[i].options[j];
					if(op.value==json['arrH'][i]['unit']){
						op.selected=true;
					}
				}
				unitKg[i].value=json['arrH'][i].unitKg;
				tmp[i].value=json['arrH'][i].tmp;
				timeRs[i].value=json['arrH'][i].timeRs;
				memo[i].value=json['arrH'][i].memo;
				i++;
			});			
		}
		if(json['arrC']!=null){
			...
			var i=0;
			$('#tbl4 input[name="mnemocode[]"]').each(function(){
				if(json['arrC'][i]==null){return false;}
				if(!json['arrC'][i].wareId){
					this.value='';
				}
				else{
					this.value=json['arrC'][i].mnemocode+':'+json['arrC'][i].wareName+' '+json['arrC'][i].guige+'        /'+json['arrC'][i].wareId;
				}
				//下拉框选项处理
				for(var j=0; j<un[i].length;j++){
					var op=un[i].options[j];
					if(op.value==json['arrC'][i]['unit']){
						op.selected=true;
					}
				}
				unitKg[i].value=json['arrC'][i].unitKg;
				tmp[i].value=json['arrC'][i].tmp;
				timeRs[i].value=json['arrC'][i].timeRs;
				memo[i].value=json['arrC'][i].memo;
				i++;
			});
		}
	});
}
// function chufangSel(obj){
// 	var url="?controller=Gongyi_Dye_Chufang&action=PopupChufang";
// 	ymPrompt.win({message:url,handler:callBack,width:800,height:500,title:'选择处方',iframe:true});
// 	return false;
...
<body>
<div id='container'>
<form name="form1" id="form1" action="{url controller=$smarty.get.controller action='saveChufang'}" method="post" onSubmit="return checkSelect();" AUTOCOMPLETE="OFF">
  <fieldset>
  <legend>处方管理</legend>
...
<table width="100%">
	<tr>
	  ...
	  <td nowrap>适用缸号：
        <select name="gangId2" id="gangId2">
          <option value="">该颜色下所有缸</option>
          {foreach from=$gangs item=aGang}
          <option value="{$aGang.id}" {if $aGang.id==$chufang.gangId}selected{/if}>{$aGang.vatNum}</option>
          {/foreach}
        </select></td>
	  <td nowrap>染色类别：
      ...
      </td><td nowrap>标记完成：<input type='checkbox' value='1' name='isComplete' id='isComplete' /></td>
    </tr>
    <tr>
    	<td nowrap colspan="1">
    		选择处方：<input type="hidden" name="checkChufang" id="checkChufang" readonly="readonly" style="width: 130px;" >
    		<!-- <input type="button" name="checkCf" id="checkCf" value="选择需要的处方数据导入" onClick="chufangSel(this)"> -->

    	</td>
    	<td nowrap colspan="2">处方性质：
    		<SELECT id="chufangKind" name="chufangKind">
    			<option value="0" {if $chufang.chufangKind=='0'}selected{/if}>正常</option>
    			<option value="1" {if $chufang.chufangKind=='1'}selected{/if}>加料</option>
    			<option value="2" {if $chufang.chufangKind=='2'}selected{/if}>修色</option>
    		</SELECT>
    	</td>
    </tr>
    {foreach from=$gangs item=aGang}
	<tr bgcolor="#CCCCCC">
		<td colspan="8">缸号:{$aGang.vatNum}
			<input type="hidden" name="gangId[]" id="gangId[]" value="{$aGang.id}">
			&nbsp;&nbsp;物理缸号:{$aGang.Vat.vatCode}
			&nbsp;&nbsp;颜色:{$aGang.OrdWare.color}
			&nbsp;&nbsp;纱支:{$aGang.Ware.wareName} {$aGang.Ware.guige}
			&nbsp;&nbsp;投料数:{$aGang.cntPlanTouliao}
			&nbsp;&nbsp;折率:<input name="zhelv[]" type="text" id="zhelv[]" value="{$aGang.zhelv}" size="5">
			&nbsp;&nbsp;水容量：
			<select name="shuirong[]" id="shuirong[]">
			<option value="{$aGang.Vat.shuirongNew}">{$aGang.Vat.shuirongNew}</option>
			...
			&nbsp;&nbsp;浴比：<span id="yubi" name="yubi" style="font-size: 13">{$aGang.Vat.shuirongNew/$aGang.cntPlanTouliao|string_format:"%.2f"}</span>
			&nbsp;&nbsp;色号：<span  style="font-size: 13">{$aGang.OrdWare.colorNum}</span>
		</td>
	</tr>
    {/foreach}
</table>
...
<!-- 	<select name="selRscf" id="selRscf" style="width:auto;" onChange="getChufang2ware()">
  	...
  	<input type="hidden" name="selRscf" id="selRscf" value="{$chufang.rscfId}" onChange="getChufang2ware()">
   	别名:<input type="text" name="memoCf" id="memoCf" value="{$chufang.memoCf}" style="width:70px;"  title='别名' emptytext="别名" placeholder="别名">
  	</legend>
<table class="tableHaveBorder table100" width="100%" id='tbl4'>
	...
	<tr>
	  ...
	  <td align="center">
	    <input name="unitKg[]" type="text" id="unitKg[]" style="width: 60px" value="{$rscf[loop4].unitKg}"  class="input100" onClick="getSel(this)">
    ...
    <tr>
	  ...
	  <td align="center">
		<input name="unitKg[]" type="text" id="unitKg[]" style="width: 60px" value="" class="input100" onClick="getSel(this)">
		<select name="unit[]" id="unit[]">
			<option value="%">%</option>
...
<!-- 	<select name="selQcl" id="selQcl" style="width:auto;" onChange="getQcl2ware()">
	...
	<span><input type="text" name="selQclName" id="selQclName" value="{$selQclName}"  style="width:80px;" readonly><input type="button" name="btnSel" id="btnSel" value=".." onClick="select1(this)"></span>
  	...
  	别名:<input type="text" name="memoQcl" id="memoQcl" value="{$chufang.memoQcl}"  style="width:70px;" title='别名' emptytext="别名" placeholder="别名">
  </legend>
<table class="tableHaveBorder table100" width="100%" id='tbl3'>
	...
	<tr>
	  ...
	  <td align="center">
	    <input name="unitKg[]" type="text" id="unitKg[]" style="width: 60px" value="{$qcl[loop3].unitKg}"  class="input100" onClick="getSel(this)">
	    <select name="unit[]" id="unit[]">
...
<legend> 染色助剂方案:  
	...
	<span><input type="text" name="selRanseName" id="selRanseName" value="{$selRanseName}"  style="width:80px;" readonly><input type="button" name="btnSel" id="btnSel" value=".." onClick="select2(this)"></span>
  	别名:<input type="hidden" name="selRanse" id="selRanse" value="{$chufang.rsgyId}" onChange="getRese2ware()">
  <input type="text" name="memoRs" id="memoRs" value="{$chufang.memoRs}" style="width:70px;" title='别名' emptytext="别名" placeholder="别名">
  </legend>
...
<!--    <td>操作</td>-->
  ...
  <tr>
    ...
    <td align="center"><input name="unitKg[]" type="text" id="unitKg[]" style="width: 60px" value="{$ranliao[loop].unitKg}"  class="input100" onClick="getSel(this)">
      <select name="unit[]" id="unit[]">
  ...
  <tr>
    <td align="center"><input style="width:80px;" name="mnemocode[]" type="text" id="mnemocode[]">
      ...
      <input name="chufang2WareId[]" type="hidden" id="chufang2WareId[]" value="">
      <input name="gongxuId[]" type="hidden" id="gongxuId[]" value="2"></td>
    <td align="center"><input name="unitKg[]" type="text" id="unitKg[]" style="width: 60px" class="input100" onClick="getSel(this)">
      <select name="unit[]" id="unit[]">
      	<option value="g/升" >g/升</option>
		  <option value="g/包">g/包</option>
		  <option value="%">%</option>
...
<legend>后处理方案:  
	...
	<span><input type="text" name="selHclName" id="selHclName" value="{$selHclName}"  style="width:80px;" readonly><input type="button" name="btnSel" id="btnSel" value=".." onClick="select3(this)"></span>
  	...
  	别名:<input type="text" name="memoHcl" id="memoHcl" value="{$chufang.memoHcl}" style="width:70px;" title='别名' emptytext="别名" placeholder="别名">
  </legend>
<table class="tableHaveBorder table100" width="100%" id='tbl2'>
	...
	<tr>
	  ...
	  <td align="center">
	    <input name="unitKg[]" type="text" id="unitKg[]" style="width: 60px" value="{$zhuji[loop2].unitKg}"  class="input100" onClick="getSel(this)">
	    <select name="unit[]" id="unit[]">
...
<div align="center">
	...
	{if $edit_model == ''} {*如果是copy就不应该传回chufangId, edit_model==''为edit模式*}
	<input type="hidden" name="chufangId" id="chufangId" value="{$chufang_id}">
	{/if}
	<input type="hidden" name="order2wareId" id="order2wareId" value="{$order2ware_id}">
