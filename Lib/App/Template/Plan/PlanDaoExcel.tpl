...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Sheet1">
  <Table x:FullColumns="1"
   ...
   <Column  ss:AutoFitWidth="0"  {if $item|@is_string==1} {else} ss:Width="{$item.width}" {/if} />
   {/foreach}
   ...
   <Row ss:AutoFitHeight="0" >
    <Cell {if $cross>1}ss:MergeAcross="{$cross-1}"{/if} ss:StyleID="s27"><Data ss:Type="String">{$date}</Data></Cell>
   </Row>
   <Row ss:AutoFitHeight="0">
   ...
   <Row ss:AutoFitHeight="0">
    ...
    <Cell ss:StyleID="{if $align[1]=='right'}s25{else}s28{/if}"><Data ss:Type="{if $align[1]=='right'}Number{else}String{/if}">{if $key2==''}{$field_value.$key|default:''}{elseif $key3==''}{$field_value.$key1.$key2|default:''}{else}{$field_value.$key1.$key2.$key3|default:''}{/if}</Data></Cell>
	{/foreach}
   </Row>
   {/foreach}
  </Table>
