...
<h2>{$title}</h2>
{*主表信息,每行显示3个字段*}
<table id="TblMainInfo" width="98%" style="BORDER-COLLAPSE: collapse" bordercolor="#000000" border="0" align="center" cellpadding="2" cellspacing="0">
  <tr id="tr">  
  <form name="form1" method="post" action="{url controller=$smarty.get.controller action='DayReport'}">
    <td width="50%" align="center">日期从：
      ...
      <input type="text" style="width:80" name="dateTo" id="dateTo" onClick="calendar()" value="{$arr_condition.dateTo}"/>      
            ...
            纱支：<input name="guige" type="text" id="guige" value="{$arr_condition.guige}" size="10"/>
            订单类型:
              ...
              <select>&nbsp;
             备注：<input name="memo" type="text" id="memo" value="{$arr_condition.memo}" size="10"/>
      <input type="submit" name="button" id="button" value="搜索"></td>
    </form>
    {*{foreach from=$arr_main_value item=item key=key} 
    <td width="50%" align="left">{$key}:{$item}</td>
     {/foreach}
    </tr> *}
</table>
{*明细列表*}
...
<br />
<input type="button" name="button" id="button" value="打印" onClick="window.print()"/>
{$other_button}
</body>
</html>
